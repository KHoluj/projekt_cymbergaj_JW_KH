...
(
    ...
    output logic goal_p1,        
    output logic goal_p2,        
    output logic puck_served,    
    output logic server_is_p2    
);
    ...
    localparam logic signed [12:0] SPEED_MAX_S = PUCK_SPEED_MAX;

    logic signed [12:0] vx, vy;
    logic [11:0] p1_x_prev, p1_y_prev, p2_x_prev, p2_y_prev;
    logic p1_overlap_prev, p2_overlap_prev;   
    ...
    function automatic logic signed [12:0] clamp_speed(input logic signed [12:0] v);
        if (v > SPEED_MAX_S)
            clamp_speed = SPEED_MAX_S;
        else if (v < -SPEED_MAX_S)
            clamp_speed = -SPEED_MAX_S;
        else
            clamp_speed = v;
    endfunction
    ...
    always_ff @(posedge clk) begin
        if (rst || !active) begin
            ...
            p1_overlap_prev <= 1'b0;
            p2_overlap_prev <= 1'b0;
        end else begin
            goal_p1 <= 1'b0;
            goal_p2 <= 1'b0;

            if (frame_tick) begin
                automatic logic signed [12:0] nx, ny, nvx, nvy;
                automatic logic signed [12:0] p1_vx, p1_vy, p2_vx, p2_vy;
                automatic logic signed [12:0] hit_offset, speed_mag;
                automatic logic in_goal_y;
                automatic logic p1_overlap_now, p2_overlap_now;
                ...
                p1_overlap_now = paddle_hit(nx[11:0], ny[11:0], p1_x, p1_y);
                p2_overlap_now = paddle_hit(nx[11:0], ny[11:0], p2_x, p2_y);

                if (p1_overlap_now && !p1_overlap_prev) begin
                    ...
                    puck_served <= 1'b1;
                end else if (p2_overlap_now && !p2_overlap_prev) begin
                ...
                p1_overlap_prev <= p1_overlap_now;
                p2_overlap_prev <= p2_overlap_now;
