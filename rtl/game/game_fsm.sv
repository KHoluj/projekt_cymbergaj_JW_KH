 ...
 *
 *   ST_SETTINGS: wejscie do ustawien
 */
...
(
    ...
    input  logic settings_click,   
    input  logic back_click,       
    input  logic menu_click,       
    input  logic exit_click,       

    input  logic [3:0] win_score,  // from settings_ctl
    input  logic [3:0] score_p1_tens, score_p1_ones,
    input  logic [3:0] score_p2_tens, score_p2_ones,

    output game_state_t game_state,
    output logic winner_p2,     // 0 = wygral gracz 1, 1 = wygral gracz 2
    output logic score_clear    
);
