...
module draw_rect_char #(
    ...
    assign scaled_x = local_x >> SCALE_SHIFT;
    assign scaled_y = local_y >> SCALE_SHIFT;

    assign char_xy   = {scaled_y[6:4], scaled_x[7:3]};
    assign char_line = scaled_y[3:0];

    logic in_rect;
    assign in_rect = active && (vga_in.hcount >= X_POS) && (vga_in.hcount < X_POS + WIDTH_PX) 
                            && (vga_in.vcount >= Y_POS) && (vga_in.vcount < Y_POS + HEIGHT_PX);
    ...
    logic [10:0] scaled_x_d2;
    logic [2:0]  pixel_x;
    logic        pixel_on;

    assign local_x_d2  = hcount_d2 - X_POS;
    assign scaled_x_d2 = local_x_d2 >> SCALE_SHIFT;

    assign pixel_x  = scaled_x_d2[2:0];
